...
<template>
	<div class="col-12 min-h-screen pl-0 pr-0 pt-0 setting-page-bg">
		<div class="ztm-container">
			...
			<div class="ztm-right browser-padding">
		...
		<div class="ztm-history browser-history-hidden"></div>
		
		<div class="px-3 mt-2">
			<!-- <div class="mt-3 text-center flex">
				<div class="flex-item pr-2">
					<Button severity="secondary" aria-haspopup="true" aria-controls="op" @click="toggle" :disabled="browser.url.length<8" class="w-full shortcut-btn-h" label="Shortcut"/>
				</div>
			</div> -->
				...
				<div class="grid text-center" v-if="shortcutApps && shortcutApps.length>0">
						<div  
							...
							<div class="ztm-icon-bg">
								<img v-longtap="()=>{removeShortcut(idx)}" :src="shortcut.icon || defaultIcon" class="pointer app-icon-img" width="40" height="40"/>
							</div>
							<div class="mt-1">
				...
				<div v-for="(history,idx) in historys" class="p-2 col-6 md:col-4" >
					<div class="p-3 rounded history-card" >
						<div class="mb-4 font-medium flex">
							<img v-if="history.icon" :src="history.icon" class="pointer app-icon-img mr-1" width="20" height="20"/>
							...
							<Button @click="removeStar(idx)" class="mesh-star-icon" icon="pi pi-star-fill" size="small" severity="secondary"/>
						</div>
						<div class="flex justify-between items-center pointer">
							<div class="mt-0 text-sm flex-item multiline-ellipsis word-break-all">
			...
			<div class="mt-4" v-if="(!shortcutApps || shortcutApps.length==0) && (!historys || historys.length==0)">
				<div class="text-center opacity-60 flex text-white-alpha-20" >
					...
					<div class="browser-dropzone">
					</div>
					</div>
				</div>
				<div class="text-center opacity-60 mx-2 text-white-alpha-80 browser-empty-msg">
