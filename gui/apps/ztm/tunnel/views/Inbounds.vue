...
const error = ref();
const getPorts = () => {
	const _ep = selectedMesh.value?.agent?.id;
	if(!!_ep){
		loading.value = true;
		loader.value = true;
		setTimeout(()=>{
...
const emptyMsg = computed(()=>{
	return `First, connect a Outbound.`
});
</script>
...
<template>
	<AppHeader :main="true">
			<template #start>
				<Button icon="pi pi-sign-in" text />
			</template>
			<template #center>
				<b>Inbounds ({{portsFilter.length}})</b>
			</template>
			<template #end> 
	...
	<Card class="nopd" v-if="!error">
		<template #content>
			<InputGroup class="search-bar" >
				...
				<Textarea @keyup="watchEnter" v-model="typing" :autoResize="true" class="drak-input bg-gray-900 text-white flex-1" placeholder="Type inbound name" rows="1" cols="30" />
			</InputGroup>
		</template>
	</Card>
