...
try {
  ...
  parseArgv(argv, {
    commands: [
      {
        title: 'Send ping requests',
        usage: '<host>',
        options: `
          -c, --count  <number>   Number of ping requests to send (default: 5)
        `,
        action: (args) => {
          var countOption = (os.platform === 'windows' ? '-n' : '-c')
          var count = Number.parseInt(args['--count'] || 5)
          if (Number.isNaN(count)) throw 'invalid count'
          println(pipy.exec(['ping', args['<host>'], countOption, count]).toString())
          pipy.exit(0)
        },
      }
    ]
